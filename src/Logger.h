...
public: /* Methods: */
    ...
    /* The following shorthands MUST stay inline as long as the user of this
       library can use LOGHARD_LOGLEVEL_MAXDEBUG to adjust the chosen helpers.
       Otherwise, if a non-default LOGHARD_LOGLEVEL_MAXDEBUG value is used by
       the user, everything will compile well, but the library will at runtime
       for some shorthands return a helper which the caller will handle using an
       incorrect type, breaking ABI. The underlying problem is that by using the
       LOGHARD_LOGLEVEL_MAXDEBUG macro, the user wants to decide which API the
       library has, but the actual dynamic library might already have been built
       using a different LOGHARD_LOGLEVEL_MAXDEBUG value, leading to a sneaky
       API/ABI mismatch. This also means that LOGHARD_LOGLEVEL_MAXDEBUG must
       always have the same value for all linked units. This is because weak
       symbols generated by the templates still share the same (mangled) names
       at (dynamic) link time, which again does not reflect their actual
       runtime semantics. */
